...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("asrafuzzamankhan89@gmail.com");
    setCopied(true);
  };
  return (
    <div
      ...
      style={{
        ...
        backgroundColor:
          "linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(9,9,121,1) 35%, rgba(4,130,173,1) 86%, rgba(0,212,255,1) 100%)",
      }}
    >
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 lg:gap-8">
                {["React.js", "Next.js", "TypeScript"].map((item) => (
                  <span
                    className="py-2 lg:py-4 lg:px-3 px-3 text-center text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg bg-[#2e1010] "
                    key={item}
                  >
                    {item}
                  </span>
              ...
              <div className="flex flex-col gap-3 lg:gap-8">
                <span className="py-4 px-3 rounded-lg text-center bg-[#10132e]" />
                {["Expres.js", "Firebase", "MongoDB"].map((item) => (
                  <span
                    className="py-2 lg:py-4 lg:px-3 px-3 text-center text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg bg-[#2e1010] "
                    key={item}
                  >
